...
namespace Trukman
{
	public class CustomCell: ViewCell
	{
		...
		public static readonly BindableProperty UserNameProperty = BindableProperty.Create("UserName", typeof(string), typeof(CustomCell), "UserName");
		public static readonly BindableProperty EmailProperty = BindableProperty.Create("Email", typeof(string), typeof(CustomCell), "Email");
			...
			removeItem.Clicked += async (sender, e) => {
				...
				if (user != null)
				{
					if (await AlertHandler.ShowCheckRemoveUser(user.UserName))
					{
						...
						await App.ServerManager.RemoveCompanyUser(user);
					}
				}
			};
