...
namespace Trukman
{
	public class PendingAuthorizPage : BasePage
	{
		Label lblSignup;
		Label lblUserRole;
		Button btnEng;
		...
		int refreshTime = 10 * 1000; // Обновление карты каждые 10 сек.

		public PendingAuthorizPage ()
		{
		...
		async void IsAuthorized(object state)
		{
			...
			var joined = await App.ServerManager.IsUserJoinedToCompany ();
			if (joined) {
				Navigation.PushModalAsync (new AuthorizedPage ());
			}
		}

		void btnCancel_Clicked (object sender, EventArgs e)
		{
			// TODO: добавить отмену заявки на регистрацию
			Navigation.PopAsync ();
		...
		void UpdateText()
		{
			...
			lblWaiting.Text = string.Format(Localization.getString(Localization.LocalStrings.WAITING_FOR_COMPANY_TO_AUTHORIZE), App.ServerManager.GetCurrentCompanyName());
