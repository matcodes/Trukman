    ...
    public class DriverContext : BaseData
    {
        ...
        private void GeoLocationChanged(GeoLocationChangedMessage message)
        {
            Task.Run(() =>
            {
                try
                {
                    if ((this.Location.Latitude != message.Latitude) || (this.Location.Longitude != message.Longitude))
                    {
                        var oldLocation = this.Location;
                        this.Location = new Position(message.Latitude, message.Longitude);
        ...
        private void DeclinedTripChanged(DeclineTripChangedMessage message)
        {
            Task.Run(() =>
            {
                ...
                try
                {
                    if (_localStorage.TripIsCancelled(this.TripID))
                        tripState = 3;
                    else
                        tripState = 2;
                }
        ...
        private void DeclineTripSubmit(DeclineTripSubmitMessage message)
        {
            Task.Run(() =>
            {
                ...
                try
                {
                    _localStorage.TripDeclined(this.TripID, message.ReasonText);
                    this.Trip = null;
                    tripState = 0;
                }
                catch (Exception exception)
        ...
        private void AcceptTripChanged(AcceptTripChangedMessage message)
        {
            Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {
                        _localStorage.TripAccepted(this.TripID);
                        tripState = 4;
                    }
                }
                catch (Exception exception)
        ...
        private void CompletedTripChanged(CompletedTripChangedMessage message)
        {
            Task.Run(() =>
            {
                ...
                try
                {
                    _localStorage.TripCompleted(this.TripID);
                    this.Trip = null;
                    this.TripID = null;
                    tripState = 0;
                }
        ...
        private void SendPhoto(SendPhotoMessage message)
        {
            Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    _localStorage.SendPhoto(this.TripID, message.Data, kind);
        ...
        private void CheckArrivedToPickup()
        {
            if ((this.Trip != null) && (this.Trip.DriverAccepted) &&
                (!this.Trip.IsPickup) && (this.DistanceToShipper < DISTANCE))
            {
                ...
                try
                {
                    ...
                    _localStorage.TripInPickup(this.TripID, (int)minutes);
        ...
        private void CheckArrivedToDelivery()
        {
            if ((this.Trip != null) && (this.Trip.DriverAccepted) &&
                ...
                (this.DistanceToReceiver < DISTANCE))
            {
                ...
                try
                {
                    ...
                    _localStorage.TripInDelivery(this.TripID, (int)minutes);
        ...
        private void TripContextChanged(int tripState)
        {
            ...
            if (!String.IsNullOrEmpty(systemMessage))
                TrukmanContext.SendSystemMessage(systemMessage);
