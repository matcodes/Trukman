...
//            ParseClient.Initialize("NsNjjvCGhqVKOZqCro2WOEr6gZHGTC9YlVB5jZqe", "WvSfa6MIvTb9L6BucGIiCQgV1zBc4OCR0UTS7D2L");
        ...
        public async Task Register(string name, string pass, UserRole role, string firstName, string lastName)
        {
            ...
            if (this.GetCurrentUserRole() == UserRole.UserRoleOwner)
                TrukmanContext.InitializeOwnerNotification();
        }
        ...
        public async Task AddCompany(string name, string DBA, string address, string phone, string email, string fleetSize)
        {
            ...
            int fs = 0;
            int.TryParse(fleetSize, out fs);

            var
                query = new ParseQuery<ParseCompany>()
                .WhereEqualTo(ServerName, lowerCaseName);
        ...
        private async Task<ParseCompany> GetCompany(string name)
        {
        ...
        public async Task<bool> _FindCompany(string name)
        {
            return await GetCompany(name.ToLower()) != null;
        }

        private async Task<bool> IsUserJoinedToCompany(ParseCompany company)
        {
            try
            {
                ...
                //    companyUsers = company.Dispatchers;
                //else if (role == UserRole.UserRoleDriver)
                //    companyUsers = company.Drivers;

                var companyUser = await company.Drivers.Query
                    .WhereEqualTo(ServerUserName, ParseUser.CurrentUser.Username)
                    .FirstOrDefaultAsync();

                if (companyUser != null)
                {
                    company.Requestings.Remove(companyUser);
                    await company.SaveAsync();
                }
        ...
        // To do: Move to External storage
        ...
        async void AddUserToRequesting(ParseCompany company)
        {
        ...
        public async void CheckRequests()
        {
            ...
            IEnumerable<ParseUser> users = await company.Requestings.Query.FindAsync();
            foreach (ParseUser user in users)
            {
                Device.BeginInvokeOnMainThread(async delegate
                {
                    ...
                    if (answer)
                    {
                        company.Requestings.Remove(user);
                        if (role == UserRole.UserRoleDispatch)
                        {
                            company.Dispatchers.Add(user);
                            await company.SaveAsync();
                        }
                        else if (role == UserRole.UserRoleDriver)
                        {
                            company.Drivers.Add(user);
                            await company.SaveAsync();
                        }
                    }
                    // Отклонена авторизация
        ...
        private async Task<ParseJob> GetTrip(string TripId)
        {
            ParseJob job = null;
        ...
        ITrip ConvertTrip(ParseObject parseData)
        {
            ...
            trip.Shipper = new Contractor
            {
                Address = GetField<string>(parseData, ServerFromAddress),
                //AddressLineSecond = GetField<string>(parseData, ServerFromAddress)
            };
            trip.Receiver = new Contractor
            {
                Address = GetField<string>(parseData, ServerToAddress),
                //AddressLineSecond = GetField<string>(parseData, ServerFromAddress)
            };
        ...
        public async Task CancelComcheckRequest(string TripId, ComcheckRequestType RequestType)
        {
            ...
            if (trip != null)
            {
                var relationQuery = trip.Advances.Query
                    .OrderByDescending("createdAt")
                    .WhereEqualTo(ServerRequestType, (int)RequestType);
                var comcheckData = await relationQuery.FirstOrDefaultAsync();
                if (comcheckData != null)
                {
                    trip.Advances.Remove(comcheckData);
                    await trip.SaveAsync();
...
[20:04:58]
        ...
        */
        ...
        public async Task<ICompany> _GetUserCompany()
        {
            ...
            if (companyData != null)
            {
                ...
                {
                    ID = companyData.ObjectId, /// ???? KAS GetField<string> (companyData, ServerObjectId),
